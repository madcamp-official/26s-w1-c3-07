...
- **즐겨찾기는 강의자 모드로 만든 노드만 가능**: 남의 수강생 모드 개인 정리 폴더까지 즐겨찾기할 수 있으면 안 되기 때문입니다(구현 방식은 [RLS 정책 → `favorites`](#favorites) 참고).
