...
export default function QuestionThread({ question, canResolve = false, onLike, onResolve, onReply, onEditReply }: QuestionThreadProps) {
  ...
  const hasLecturerReply = question.replies.some((reply) => reply.authorRole === 'lecturer')
  ...
  return (
    <article className={cn('rounded-3xl border bg-white p-5 shadow-sm sm:p-6', question.isResolved ? 'border-slate-100' : (isLecturer || hasLecturerReply) ? 'border-blue-200' : isOpinion ? 'border-rose-200' : 'border-violet-200')}>
