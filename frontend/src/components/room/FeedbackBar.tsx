...
export default function FeedbackBar({ options, onVote }: FeedbackBarProps) {
  return (
      ...
      {options.map((option) => (
        <div key={option.key} className="inline-flex items-center gap-2 rounded-full border border-amber-200 bg-white px-3 py-1.5 text-sm font-bold text-slate-700">
          ...
          <button
            ...
            className={cn('inline-flex items-center gap-1 rounded-full px-1.5 py-0.5 transition', option.myVote === 'like' ? 'bg-emerald-100 text-emerald-700' : 'text-slate-400 hover:text-emerald-600')}
          ...
          <button
            ...
            className={cn('inline-flex items-center gap-1 rounded-full px-1.5 py-0.5 transition', option.myVote === 'dislike' ? 'bg-rose-100 text-rose-700' : 'text-slate-400 hover:text-rose-600')}
