...
- [RLS 설정 및 guest_token 관련](#rls-설정-및-guest_token-관련)
  ...
  - [4. 비회원 익명 식별자(`guest_token`) 생성 로직](#4-비회원-익명-식별자guest_token-생성-로직)
  - [5. `x-guest-token` 커스텀 헤더를 실제로 보내는 구현](#5-x-guest-token-커스텀-헤더를-실제로-보내는-구현)
  - [6. `posts_public` 뷰로 조회 대상 전환](#6-posts_public-뷰로-조회-대상-전환)
- [AI 보조 기능 관련 (Edge Function vs Express 서버 미결정)](#ai-보조-기능-관련-edge-function-vs-express-서버-미결정)
  - [7. 유사도 검사 비교 대상 범위](#7-유사도-검사-비교-대상-범위-미해결-게시글만-답글도-포함)
  - [8. 유사도 비교를 위한 벡터 컬럼 추가 여부 결정](#8-유사도-비교를-위한-벡터-컬럼-추가-여부-결정)
  - [9. AI 보조 기능 서버 아키텍처 결정](#9-ai-보조-기능-서버-아키텍처-결정-edge-function-vs-express)
- [Realtime 관련](#realtime-관련)
  - [10. `max_participants`(최다 참여 인원) 강제 여부 결정](#10-max_participants최다-참여-인원-강제-여부-결정)
...
### 7. 유사도 검사 비교 대상 범위 (미해결 게시글만? 답글도 포함?)
...
### 9. AI 보조 기능 서버 아키텍처 결정 (Edge Function vs Express)

AI 교정, 부적절한 내용 필터링, 유사 질문 자동 탐지(#7, #8) 기능을 처리할 서버를 **Supabase Edge Function**으로 만들지, **별도 Express 서버**를 새로 띄울지 아직 결정 안 됨. 현재는 Express 백엔드 서버 없이 Supabase만으로 구성되어 있음(`DB_DESIGN.md` 배포 현황 참고). 이 결정에 따라 배포 방식, 인증 처리(Edge Function은 Supabase 세션과 통합이 쉬움), LLM API 키 보관 위치 등이 달라짐.

## Realtime 관련

### 10. `max_participants`(최다 참여 인원) 강제 여부 결정
