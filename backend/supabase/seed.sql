-- 데모/테스트용 더미 데이터.
-- profiles는 새로 만들지 않고 이미 가입된 실제 계정 3개를 재사용합니다
-- (auth.users에 FK가 걸려 있어 가짜 auth.users를 직접 만드는 건 피함).
--   A = b316c95c-796e-4570-9016-bdb06357bac3 (조준호) — "그래프 이론"/"DB 설계" 강의자
--   B = c444644b-4fc9-4f5b-b6c8-506826690f7c (Junho Jo)  — "배열과 연결 리스트"/"AI 세미나"/"파이썬 워크샵" 강의자
--   C = 0833a865-2786-4dde-a7c0-e64f3b8feea3 (xxx)       — "커리어 토크" 강의자
...
insert into nodes (id, parent_id, node_type, name, created_by, created_mode, created_at) values
  ('f0000000-0000-0000-0000-000000000001', null, 'folder', '그래프 이론 강의', 'b316c95c-796e-4570-9016-bdb06357bac3', 'lecturer', now() - interval '10 days'),
  ('f0000000-0000-0000-0000-000000000002', 'f0000000-0000-0000-0000-000000000001', 'lecture', '트리와 그래프', 'b316c95c-796e-4570-9016-bdb06357bac3', 'lecturer', now() - interval '3 days'),
  ('f0000000-0000-0000-0000-000000000003', null, 'lecture', '데이터베이스 설계 입문', 'b316c95c-796e-4570-9016-bdb06357bac3', 'lecturer', now() - interval '1 day');

-- ===== nodes: A가 수강생 모드로 만든 개인 정리 폴더 =====
insert into nodes (id, parent_id, node_type, name, created_by, created_mode, created_at) values
  ('f0000000-0000-0000-0000-000000000004', null, 'folder', '컴퓨터 과학', 'b316c95c-796e-4570-9016-bdb06357bac3', 'student', now() - interval '9 days'),
...
insert into my_nodes (user_id, node_id, folder_id) values
  ('b316c95c-796e-4570-9016-bdb06357bac3', 'f0000000-0000-0000-0000-000000000006', 'f0000000-0000-0000-0000-000000000005'), -- 배열과 연결 리스트 -> 자료구조 폴더 안에
  ('b316c95c-796e-4570-9016-bdb06357bac3', 'f0000000-0000-0000-0000-000000000008', 'f0000000-0000-0000-0000-000000000004'), -- 운영체제 -> 컴퓨터 과학 폴더 안에
  ('b316c95c-796e-4570-9016-bdb06357bac3', 'f0000000-0000-0000-0000-000000000009', null), -- 수학 -> 최상위
  ('b316c95c-796e-4570-9016-bdb06357bac3', 'f0000000-0000-0000-0000-00000000000a', null), -- AI 세미나 -> 최상위
  ('b316c95c-796e-4570-9016-bdb06357bac3', 'f0000000-0000-0000-0000-00000000000b', null), -- 커리어 토크 -> 최상위
  ('b316c95c-796e-4570-9016-bdb06357bac3', 'f0000000-0000-0000-0000-00000000000c', null); -- 파이썬 워크샵 -> 최상위
...
insert into posts (id, lecture_id, parent_id, author_id, is_anonymous, guest_token, post_type, status, content, created_mode, created_at) values
  ('e0000000-0000-0000-0000-000000000002', 'f0000000-0000-0000-0000-000000000002', 'e0000000-0000-0000-0000-000000000001', 'b316c95c-796e-4570-9016-bdb06357bac3', false, null, 'opinion', null, '좋은 질문입니다! DFS는 트리 높이 h만큼의 공간(O(h))을 사용하고, BFS는 최대 너비 w만큼의 큐를 유지합니다(O(w)). 균형 이진 트리에서 h ≈ log n이므로 DFS가 훨씬 효율적입니다. 반면 최단 경로를 찾아야 할 때는 BFS가 필수입니다.', 'lecturer', now() - interval '48 minutes'),
...
-- 최상위 질문 2 (실명, 미해결)
insert into posts (id, lecture_id, parent_id, author_id, is_anonymous, guest_token, post_type, status, content, created_mode, created_at) values
  ('e0000000-0000-0000-0000-000000000005', 'f0000000-0000-0000-0000-000000000002', null, '0833a865-2786-4dde-a7c0-e64f3b8feea3', false, null, 'question', 'unresolved', '무방향 그래프에서 DFS로 사이클을 감지하는 방법이 방향 그래프와 어떻게 다른가요?', 'student', now() - interval '25 minutes');
...
insert into posts (id, lecture_id, parent_id, author_id, is_anonymous, guest_token, post_type, status, content, created_mode, created_at) values
  ('e0000000-0000-0000-0000-000000000007', 'f0000000-0000-0000-0000-000000000002', 'e0000000-0000-0000-0000-000000000006', 'b316c95c-796e-4570-9016-bdb06357bac3', false, null, 'opinion', null, '사이클이 있으면 루트에서 특정 노드까지 가는 경로가 여러 개가 되어 트리의 계층 구조가 깨지기 때문입니다.', 'lecturer', now() - interval '38 minutes');

-- 최상위 질문 4 (실명, 해결됨, 답글 없음)
insert into posts (id, lecture_id, parent_id, author_id, is_anonymous, guest_token, post_type, status, resolved_at, content, created_mode, created_at) values
  ('e0000000-0000-0000-0000-000000000008', 'f0000000-0000-0000-0000-000000000002', null, 'c444644b-4fc9-4f5b-b6c8-506826690f7c', false, null, 'question', 'resolved', now() - interval '55 minutes', '최소 신장 트리에서 크루스칼과 프림 알고리즘의 시간 복잡도 차이를 다시 설명해 주실 수 있나요?', 'student', now() - interval '1 hour');
...
insert into post_likes (post_id, voter_key) values
  ...
  ('e0000000-0000-0000-0000-000000000001', '0833a865-2786-4dde-a7c0-e64f3b8feea3'),
...
insert into lecture_feedback_votes (lecture_id, feedback_type, voter_key, value) values
  ...
  ('f0000000-0000-0000-0000-000000000002', 'cold', '0833a865-2786-4dde-a7c0-e64f3b8feea3', -1),
