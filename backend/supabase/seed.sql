...
--   C = 0c58fd0c-8d15-4ed8-a546-134537590d8c (안소희3) — "미적분과 선형대수"/"스택과 큐"/"정렬과 탐색" 강의자
-- A는 강의자 모드(자기 강의 2개)와 수강생 모드(다른 사람 강의/폴더 즐겨찾기)를 둘 다 사용하는 계정으로 구성.

begin;

-- ===== nodes: A가 강의자 모드로 만든 폴더/강의 =====
insert into nodes (id, parent_id, type, name, created_by, created_mode, created_at) values
...
-- 즐겨찾기(favorites)는 강의자 모드로 만든 노드만 등록 가능하므로, 운영체제 폴더도
...
-- 즐겨찾기(favorites)는 강의자 모드로 만든 노드만 등록 가능하므로, 수학 폴더도
...
-- ===== lectures: 강의 부가 속성 =====
insert into lectures (id, start_time, end_time, location, max_participants) values
...
-- ===== favorites: A가 수강생 모드에서 즐겨찾기한 것들 =====
insert into favorites (user_id, node_id, anchor_id) values
...
insert into posts (id, lecture_id, parent_id, author_id, is_anonymous, guest_token, type, status, content, created_mode, created_at) values
...
insert into posts (id, lecture_id, parent_id, author_id, is_anonymous, guest_token, type, status, resolved_at, content, created_mode, created_at) values
...
insert into lecture_feedback_votes (lecture_id, feedback_type, voter_key, value) values
  ...
  ('f0000000-0000-0000-0000-000000000002', 'unclear', 'd0000000-0000-0000-0000-000000000001', 1),
