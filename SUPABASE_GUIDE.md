...
`backend/supabase/seed.sql`에 실제 스키마에 맞춘 더미 데이터가 원격 DB에 반영되어 있어요(강의 폴더/강의, 질문/답글, 좋아요, 실시간 피드백 등). 강의 입장 코드로 바로 테스트해볼 수 있어요. 계정별로 어떤 강의/폴더가 어떻게 보이는지는 [`DUMMY_DATA.md`](./DUMMY_DATA.md) 참고:
