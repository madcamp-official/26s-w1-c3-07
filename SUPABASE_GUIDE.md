...
"내 강의"/"강의" 페이지에서 서버에 물어봐야 하는 건 크게 세 가지예요. (1) 강의자 모드로 "내 강의"에 접속했을 때 내가 만든 모든 노드, (2) 수강생 모드로 "내 강의"에 접속했을 때 내가 만든 노드와 즐겨찾기한 노드들의 서브트리, (3) 강의 페이지에 접속했을 때 해당 `lecture_id`의 모든 `posts`입니다. 이 중 (1)·(3)은 평평한 필터 조회라 RPC 없이 바로 조회하면 되고, (2)만 재귀 조회가 필요해서 RPC를 써야 해요.

**⚠️ 아직 프론트 코드에 구현되어 있지 않습니다** — 아래 RPC/RLS는 백엔드 쪽에서 이미 만들어 원격 DB에 반영해뒀지만, 프론트 코드에서 실제로 이 패턴을 쓰는 곳은 아직 없어요([TODO.md 프론트엔드 참고](./TODO.md#프론트엔드)).
...
Edge Function은 필요 없습니다 — 순수 DB 조회라 RPC로 완결되고, 세 요청 다 로그인 시 한꺼번에 받아두지 말고 모드 전환/페이지 진입 시점마다 그 모드에 맞는 것만 요청하면 됩니다.
