...
**⚠️ `frontend` 브랜치에 이미 만들어진 폴더 트리/드래그앤드롭 UI가 있다면 이 섹션 말고 그쪽 구현을 따르세요.** 여기 적힌 색 태깅·트리 조립·드래그앤드롭 제약 방식은 백엔드 스키마/RLS를 근거로 유도해본 예시일 뿐, 실제로 프론트에서 이미 다르게 구현해뒀다면 그게 우선입니다. 이 섹션은 그럴 때 참고할 배경 설명(왜 겹침이 안 생기는지, 파란 덩어리 이동이 왜 이미 DB에서 막히는지) 정도로만 활용하세요.
